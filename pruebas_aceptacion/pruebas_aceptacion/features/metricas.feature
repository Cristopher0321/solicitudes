Feature: Dashboard administrativo
    ...
    Scenario: El administrador visualiza el dashboard con métricas principales
        Cuando el administrador inicia sesión y abre "/tipo-solicitud/admin/dashboard/"
        Entonces debería ver el título "Dashboard administrativo"
    ...
    Scenario: Un usuario no administrador no puede acceder al dashboard administrativo
        Dado que existe un usuario normal con usuario "usuario" y contraseña "userpass"
        Cuando el usuario inicia sesión y abre "/tipo-solicitud/admin/dashboard/"
